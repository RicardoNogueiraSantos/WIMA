#include "/home/user/WIMA/samples/Contiki/client/include/Slicer.h"
...
tSlicedData* tSlicer_sliceImage(tImage * img) {
	...
	if (img != NULL) {
		...
		if (datasize%TRANSFER_DATA_BUFFER_SIZE != 0) {
			...
			sdata->lastSliceSize = datasize%TRANSFER_DATA_BUFFER_SIZE; //� o que falta pra completar os dados
		} else {
			//sem sobra no final, dados encaixam perfeito nos slices
			sdata->slices = datasize/TRANSFER_DATA_BUFFER_SIZE; //slices
			sdata->sliceSize = TRANSFER_DATA_BUFFER_SIZE;
			sdata->lastSliceSize = sdata->sliceSize; //o �ltimo � do mesmo tamanho dos outros
		}
		...
		int sliceIndex = 0; //marca em qual slice est�
		int dataIndex = 0; //marca em qual indice dentro do slice est�
...
tSlicedData* tSlicer_sliceCompressedData(tCompressedData * compressedData) {
	...
	if (compressedData != NULL) {
		...
		if (compressedData->size%TRANSFER_DATA_BUFFER_SIZE != 0) {
			...
			sdata->sliceSize = TRANSFER_DATA_BUFFER_SIZE;
			sdata->lastSliceSize = sdata->slices*sdata->sliceSize - compressedData->size; //� o que falta pra completar os dados
		} else {
			//sem sobra no final, dados encaixam perfeito nos slices
